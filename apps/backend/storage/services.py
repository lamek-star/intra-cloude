...
class PreparedUpload:
    """Server-produced upload receipt; never accepted from an API payload."""
...
def prepare_upload(*, uploaded_file, file_id: uuid.UUID, object_key: str) -> PreparedUpload:
    """Validate, scan and upload bytes without holding a catalog transaction.

    Keys and IDs are generated by trusted service code, never archive paths.
    Restore uses a stable job-owned key so interrupted uploads can be repeated.
    """
    checksum, size, mime_type = _hash_and_sniff(uploaded_file)
    uploaded_file.seek(0)
    is_clean = _scan_for_malware(uploaded_file)
    get_client().put_stream(object_key, uploaded_file, mime_type)
    return PreparedUpload(file_id, object_key, uploaded_file.name, checksum, size, mime_type, is_clean)


def publish_upload(
    *, bucket: Bucket, folder: Folder | None, prepared: PreparedUpload, display_filename: str, creator
) -> FileObject:
    """Publish a trusted upload receipt and its audit event atomically."""
    with transaction.atomic():
        return _publish_upload(
            bucket=bucket,
            folder=folder,
            prepared=prepared,
...
def _publish_upload(*, bucket, folder, prepared: PreparedUpload, display_filename: str, creator):
    file_status = FileObject.Status.ACTIVE if prepared.is_clean else FileObject.Status.QUARANTINED
    ...
    audit.record(
        ...
        result=AuditEvent.Result.SUCCESS if prepared.is_clean else AuditEvent.Result.DENIED,
        context={
            "display_filename": display_filename,
            "size": prepared.size,
            "quarantined": not prepared.is_clean,
        },
    )
    return file_obj
...
def upload_file(
    *, bucket: Bucket, folder: Folder | None, uploaded_file, display_filename: str, creator
    ...
    prepared = prepare_upload(
        ...
        file_id=file_id,
        object_key=_new_object_key(bucket.organization_id, bucket.project_id, bucket.id, file_id),
    )
    return publish_upload(
        bucket=bucket, folder=folder, prepared=prepared, display_filename=display_filename, creator=creator
    )
