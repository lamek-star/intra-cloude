...
- [ ] Confirm `/readyz` reports healthy immediately after a real restore, and separately verify `Invoke-IntraCloudRestore.ps1`'s implemented post-restore wait through the Control Center. Its container-status polling is not equivalent to exercising every dependency or user workflow.
